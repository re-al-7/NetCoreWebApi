...
namespace Integrate.Utils
{
    public class cFuncionesZip
    {
        ...
        public static cFuncionesZip Create(string _filename, string _comment)
        {
            Stream stream = new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            cFuncionesZip zip = Create(stream, _comment);
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static cFuncionesZip Create(Stream _stream, string _comment)
        {
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static cFuncionesZip Open(Stream _stream, FileAccess _access)
        {
        ...
        public static bool RemoveEntries(ref cFuncionesZip _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                cFuncionesZip tempZip = cFuncionesZip.Create(tempZipName, string.Empty);
                ...
                _zip = cFuncionesZip.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        _zfe.Crc32 = cFuncionesZip.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    }
                }
