...
{
	public class RnSegPaginas
	{
		private string strMiToken = "";
		
		public RnSegPaginas(string strToken = "")
		{
		...
		public EntSegPaginas ObtenerObjeto(int intpaginaspg)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync(CParametros.StrApiTables + EntSegPaginas.StrNombreTabla + "/" + intpaginaspg).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
					obj = JsonConvert.DeserializeObject<EntSegPaginas>(stringData);
				}
				else
		...
		{
			var data = new List<EntSegPaginas>();
			using (HttpClient client = new HttpClient())
			{
				client.BaseAddress = new Uri(CParametros.StrBaseUri);
				MediaTypeWithQualityHeaderValue contentType = new MediaTypeWithQualityHeaderValue("application/json");
				...
				HttpResponseMessage response = client.GetAsync(CParametros.StrApiTables + EntSegPaginas.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
					data = JsonConvert.DeserializeObject <List<EntSegPaginas>>(stringData);
				}
				else
					throw new CApiExcepcion(response);
			}
		...
		/// <param name="bValidar">Valor que indica si se debe realizar la validación en base a los DataAnnotation</param>
		/// <returns>Valor TRUE or FALSE que indica el exito de la operacion</returns>
		public bool Update(EntSegPaginas obj, bool bValidar = true)
		{
			bool bProcede = false;
			using (HttpClient client = new HttpClient())
			{
		...
		/// <param name="bValidar">Valor que indica si se debe realizar la validación en base a los DataAnnotation</param>
		/// <returns>Valor TRUE or FALSE que indica el exito de la operacion</returns>
		public bool Delete(EntSegPaginas obj, bool bValidar = true)
		{
			bool bProcede = false;
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync(CParametros.StrApiTables+ EntSegPaginas.StrNombreTabla +"/" + obj.paginaspg).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
