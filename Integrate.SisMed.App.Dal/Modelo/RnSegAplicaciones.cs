	...
	public class RnSegAplicaciones
	{
		...
		public EntSegAplicaciones ObtenerObjeto(String Stringaplicacionsap)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegAplicaciones.StrNombreTabla + "/" + Stringaplicacionsap).Result;
		...
		public List<EntSegAplicaciones> ObtenerLista()
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegAplicaciones.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
		...
		public bool Delete(EntSegAplicaciones obj, bool bValidar = true)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync("/api/values/"+ EntSegAplicaciones.StrNombreTabla +"/" + obj.aplicacionsap).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
