#region 
/***********************************************************************************************************
	NOMBRE:       RnSegTransiciones
	DESCRIPCION:
		Clase que implmenta los metodos y operaciones sobre la Tabla segtransiciones
	...
	public class RnSegTransiciones
	{
		...
		public EntSegTransiciones ObtenerObjeto(String Stringtablasta, String Stringestadoinicialsts, String Stringtransaccionstr, String Stringestadofinalsts)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegTransiciones.StrNombreTabla + "/" + Stringtablasta+ "/" + Stringestadoinicialsts+ "/" + Stringtransaccionstr+ "/" + Stringestadofinalsts).Result;
		...
		public List<EntSegTransiciones> ObtenerLista()
		{
			var data = new List<EntSegTransiciones>();
			using (HttpClient client = new HttpClient())
			{
				client.BaseAddress = new Uri(CParametros.strBaseUri);
				MediaTypeWithQualityHeaderValue contentType = new MediaTypeWithQualityHeaderValue("application/json");
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegTransiciones.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
					data = JsonConvert.DeserializeObject <List<EntSegTransiciones>>(stringData);
				}
				else
					throw new CApiExcepcion(response);
			}
		...
		public bool Insert(EntSegTransiciones obj, bool bValidar = true)
		{
			bool bProcede = false;
			using (HttpClient client = new HttpClient())
			{
		...
		public bool Delete(EntSegTransiciones obj, bool bValidar = true)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync("/api/values/"+ EntSegTransiciones.StrNombreTabla +"/" + obj.tablasta+ "/" + obj.estadoinicialsts+ "/" + obj.transaccionstr+ "/" + obj.estadofinalsts).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
