	...
	public class RnSegTransacciones
	{
		...
		public EntSegTransacciones ObtenerObjeto(String Stringtablasta, String Stringtransaccionstr)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegTransacciones.StrNombreTabla + "/" + Stringtablasta+ "/" + Stringtransaccionstr).Result;
		...
		public List<EntSegTransacciones> ObtenerLista()
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegTransacciones.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
		...
		public bool Delete(EntSegTransacciones obj, bool bValidar = true)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync("/api/values/"+ EntSegTransacciones.StrNombreTabla +"/" + obj.tablasta+ "/" + obj.transaccionstr).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
