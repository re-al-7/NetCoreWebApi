	...
	public class RnSegParametrosdet
	{
		...
		public EntSegParametrosdet ObtenerObjeto(String Stringsiglaspa, int intcodigospd)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegParametrosdet.StrNombreTabla + "/" + Stringsiglaspa+ "/" + intcodigospd).Result;
		...
		public List<EntSegParametrosdet> ObtenerLista()
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegParametrosdet.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
		...
		public bool Delete(EntSegParametrosdet obj, bool bValidar = true)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync("/api/values/"+ EntSegParametrosdet.StrNombreTabla +"/" + obj.siglaspa+ "/" + obj.codigospd).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
