#region 
/***********************************************************************************************************
	NOMBRE:       RnSegRolestablastransacciones
	DESCRIPCION:
		Clase que implmenta los metodos y operaciones sobre la Tabla segrolestablastransacciones
...
{
	public class RnSegRolestablastransacciones
	{
		private string strMiToken = "";
		
		public RnSegRolestablastransacciones(string strToken = "")
		{
		...
		public EntSegRolestablastransacciones ObtenerObjeto(int introlsro, String Stringtablasta, String Stringtransaccionstr)
		{
			var obj = new EntSegRolestablastransacciones();
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync(CParametros.StrApiTables + EntSegRolestablastransacciones.StrNombreTabla + "/" + introlsro+ "/" + Stringtablasta+ "/" + Stringtransaccionstr).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
					obj = JsonConvert.DeserializeObject<EntSegRolestablastransacciones>(stringData);
				}
				else
		...
		public List<EntSegRolestablastransacciones> ObtenerLista()
		{
			var data = new List<EntSegRolestablastransacciones>();
			using (HttpClient client = new HttpClient())
			{
				client.BaseAddress = new Uri(CParametros.StrBaseUri);
				MediaTypeWithQualityHeaderValue contentType = new MediaTypeWithQualityHeaderValue("application/json");
				...
				HttpResponseMessage response = client.GetAsync(CParametros.StrApiTables + EntSegRolestablastransacciones.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
					data = JsonConvert.DeserializeObject <List<EntSegRolestablastransacciones>>(stringData);
				}
				else
					throw new CApiExcepcion(response);
			}
		...
		public bool Insert(EntSegRolestablastransacciones obj, bool bValidar = true)
		{
			bool bProcede = false;
			using (HttpClient client = new HttpClient())
			{
		...
		/// <param name="bValidar">Valor que indica si se debe realizar la validación en base a los DataAnnotation</param>
		/// <returns>Valor TRUE or FALSE que indica el exito de la operacion</returns>
		public bool Update(EntSegRolestablastransacciones obj, bool bValidar = true)
		{
			bool bProcede = false;
			using (HttpClient client = new HttpClient())
			{
		...
		/// <param name="bValidar">Valor que indica si se debe realizar la validación en base a los DataAnnotation</param>
		/// <returns>Valor TRUE or FALSE que indica el exito de la operacion</returns>
		public bool Delete(EntSegRolestablastransacciones obj, bool bValidar = true)
		{
			bool bProcede = false;
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync(CParametros.StrApiTables+ EntSegRolestablastransacciones.StrNombreTabla +"/" + obj.rolsro+ "/" + obj.tablasta+ "/" + obj.transaccionstr).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
