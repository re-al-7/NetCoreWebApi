...
namespace Integrate.SisMed.App.Dal.Modelo
{
	public class RnSegEstados
	{
		...
		public RnSegEstados(string strToken = "")
		{
		...
		public EntSegEstados ObtenerObjeto(String Stringtablasta, String Stringestadoses)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegEstados.StrNombreTabla + "/" + Stringtablasta+ "/" + Stringestadoses).Result;
		...
		public List<EntSegEstados> ObtenerLista()
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.GetAsync("/api/values/" + EntSegEstados.StrNombreTabla).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var stringData = response.Content.ReadAsStringAsync().Result;
		...
		public bool Delete(EntSegEstados obj, bool bValidar = true)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				HttpResponseMessage response = client.DeleteAsync("/api/values/"+ EntSegEstados.StrNombreTabla +"/" + obj.tablasta+ "/" + obj.estadoses).Result;
				if (response.StatusCode == HttpStatusCode.OK)
				{
					var strResult = response.Content.ReadAsStringAsync().Result;
					bProcede = true;
